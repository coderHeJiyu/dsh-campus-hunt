...
*/
function resolveEntryValue(value) {
...
function apply(ctx, config) {
	...
	if (settings !== void 0) {
		const reread = () => {
			...
			if (JSON.stringify(parsed) === JSON.stringify(currentConfig())) return;
			setSource(() => parsed);
			resyncSkill();
		};
		ctx.on("settings/document-updated", (ns, _revision) => {
			if (ns === "dsh-campus-hunt") reread();
		});
...
//#endregion
export { Config, NS, PLUGIN_VERSION, apply, currentConfig, defaultConfig, inject, isDomainAllowed, name, resolveEntryValue, setSource };
